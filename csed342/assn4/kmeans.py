...
def euclidean_distance(dp1, dp2):
    ...
    """
    # BEGIN_YOUR_ANSWER
    return np.linalg.norm(np.subtract(dp1, dp2))
    # END_YOUR_ANSWER
...
def update_assignment(data, centroids):
    ...
    points = {}
    for dp in data:
        key = assign_data(dp, centroids)
        if key not in points:
            points[key] = []
        points[key].append(dp)
    return points
    # END_YOUR_ANSWER
...
def update_centroids(assignment_dict):
    ...
    for key, data in assignment_dict.items():
        updated_centroids[key] = mean_of_points(data)
